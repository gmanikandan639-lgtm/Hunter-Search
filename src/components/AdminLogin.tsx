...
export const AdminLogin: React.FC<AdminLoginProps> = ({
  ...
  const handleAuthCompleted = (user: any, profile: { isAdmin: boolean; role: string; name: string }) => {
    if (profile.role === 'admin' || profile.isAdmin) {
      ...
      setErrorMessage('Access Denied: This portal is strictly restricted to system administrators (role="admin"). Public users can access Hunter Search directly without logging in.');
    }
  };
        ...
        >
          ...
          <div className="text-center space-y-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {mode === 'login' && 'Sign in with system administrator credentials'}
          ...
          {mode === 'login' && (
            <div className="space-y-4">
              ...
              {/* Public access info and Return to Hunter Search */}
              <div className="pt-2 text-center border-t border-slate-100 flex flex-col items-center gap-2">
                <p className="text-[11px] text-slate-500">
                  Public users can access Hunter Search directly without authentication.
                </p>
                {onCancel && (
                  <button
